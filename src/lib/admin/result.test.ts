...
describe("adminError", () => {
  it("recognises the database's publish and consent guards", () => {
    expect(adminError({ code: "23514", message: "Cannot publish audio: the speaker has no recorded consent" }, "audio")).toBe("consent_required");
    expect(adminError({ code: "23514", message: "Entry x is used by a published lesson or quiz; unpublish that first" })).toBe("in_use");
    expect(adminError({ code: "23514", message: "Cannot publish level: its lesson or quiz is still a draft" })).toBe("in_use");
  });
  ...
  it("maps permissions, duplicates and references", () => {
    ...
    expect(adminError({ code: "23503" }, "speaker")).toBe("has_audio");
    expect(adminError({ code: "23503" }, "entry")).toBe("in_use");
